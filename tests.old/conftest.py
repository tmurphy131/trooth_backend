...
def mock_admin(monkeypatch):
    ...
    monkeypatch.setattr("app.auth.get_current_user", _get_mock_admin)

@pytest.fixture
def mock_apprentice(monkeypatch):
    def _get_mock_apprentice():
        return MockUser(role="apprentice")
    monkeypatch.setattr("app.auth.get_current_user", _get_mock_apprentice)

@pytest.fixture
def mock_mentor(monkeypatch):
    def _get_mock_mentor():
        return MockUser(role="mentor")
    monkeypatch.setattr("app.auth.get_current_user", _get_mock_mentor)
...
def mentor_user(db_session):
    user = User(
        id="mentor-1",
        name="Mentor One",
        email=random.choice(["tay.murphy88@gmail.com", "tay.murphy88@yahoo.com", "taylor.murphy@cox.com"]),
        role="mentor",
